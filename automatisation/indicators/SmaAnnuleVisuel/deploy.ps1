# Compile l'indicateur visuel H3 (net10.0) et le deploie dans Settings\Scripts\Indicators.
...
dotnet build "$PSScriptRoot\SmaAnnuleVisuel.csproj" -c Release -p:QuantowerBin="$bin"
...
$dest = Join-Path $scripts "SmaAnnuleVisuel"
New-Item -ItemType Directory -Force -Path $dest | Out-Null
foreach ($f in "SmaAnnuleVisuel.dll","SmaAnnuleVisuel.deps.json","SmaAnnuleVisuel.pdb") {
