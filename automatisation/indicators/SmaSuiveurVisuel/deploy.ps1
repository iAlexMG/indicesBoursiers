# Compile l'indicateur visuel H2 (net10.0) et le deploie dans Settings\Scripts\Indicators.
...
dotnet build "$PSScriptRoot\SmaSuiveurVisuel.csproj" -c Release -p:QuantowerBin="$bin"
...
$dest = Join-Path $scripts "SmaSuiveurVisuel"
New-Item -ItemType Directory -Force -Path $dest | Out-Null
foreach ($f in "SmaSuiveurVisuel.dll","SmaSuiveurVisuel.deps.json","SmaSuiveurVisuel.pdb") {
