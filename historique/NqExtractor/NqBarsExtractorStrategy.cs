...
public sealed class NqBarsExtractorStrategy : Strategy
{
    ...
    [InputParameter("Base SQLite (vide = auto H:\\IndicesBoursiers\\historique\\<sym>-<contrat>-1m.db)", 1)]
    ...
    private string ResolveDbPath(Symbol s)
    {
        ...
        string root = string.IsNullOrWhiteSpace(s.Root) ? "NQ" : s.Root;
        string tag = s.ExpirationDate.ToString("yyyy-MM", CultureInfo.InvariantCulture);
        return $@"H:\IndicesBoursiers\historique\{root}-{tag}-{PeriodMinutes}m.db";
    }
